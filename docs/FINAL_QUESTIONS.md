...
`make experiment-prompt-v1` / `make experiment-prompt-v2` (Milestone 10) run the same 39-case dataset under each `PROMPT_LABEL`, publishing each as a named Langfuse Dataset Experiment run — open both `dataset_run_url`s and select one as a comparison baseline in Langfuse's own UI for a native side-by-side view, scores included. **Under the default `MockProvider`**, prompt *content* is structurally invisible to the model's own decision logic (`_decide()` only reads the last user message) — Milestone 21's final suite confirmed this live, two prompt-version configs scoring identically to two decimal places on every metric. **Under a real model** (`LLM_PROVIDER=anthropic`), the two prompts finally do score differently — but the real, live finding (see [docs/EXPERIMENTS.md](EXPERIMENTS.md), "Real Anthropic provider, at last") is more interesting than "v2 wins": three separate real-model runs of nominally the same comparison disagreed on which prompt was ahead, by margins comparable to the run-to-run noise on a single unchanged config. A single real-model run cannot reliably separate a true prompt effect from ordinary model stochasticity — this project's evaluation harness runs each case exactly once per config, so it has no built-in way to average that noise out. Repeated trials with a variance/significance check would be the natural next step, and are not built here.
