...
### Travel AI Search Integration (M18)

`TravelSearchProvider` — a Protocol mirroring `LLMProvider` (ADR-003) exactly, so the travel tools can be served by a separately running Travel AI Search backend instead of this project's own local synthetic dataset, without creating a hard dependency on it. See [ADR-006](decisions/ADR-006-travel-search-provider-abstraction.md) for the full design reasoning and [docs/EXPERIMENTS.md](EXPERIMENTS.md) for the live end-to-end demonstration.

- **`providers/travel_search/`** — `base.py` (the Protocol: `search_destinations`/`search_hotels`/`get_destination_information`, same signatures as the tool functions themselves), `local.py` (`LocalSyntheticTravelSearchProvider`, the always-available default — the exact filtering logic that used to live directly in `tools/travel_tools.py`), `api.py` (`TravelAISearchAPIProvider`, calls the separate backend over `httpx2`), `data.py` (the JSON loader, moved here from `tools/data.py` — see below), `__init__.py` (`get_travel_search_provider()`, `lru_cache`'d, selects from `Settings.travel_search_provider`, default `"local"`).
- **Deliberately synchronous, unlike `LLMProvider`** — `agent/nodes.py`'s `tools_node` calls every `TOOL_REGISTRY` entry as a plain sync call, never awaited, a convention every tool since M4 depends on. Making the Protocol async would mean making the whole tool-execution path async too, a much larger change than this milestone needs.
- **`tools/travel_tools.py`'s three functions are now thin wrappers** — open the same Langfuse `tool` span as before (unchanged since M4), delegate to `get_travel_search_provider()`, record the result. `TOOL_SPECS`/`TOOL_REGISTRY`/`agent/nodes.py` needed zero changes — the same "swap the concrete provider, not the call site" pattern `LLMProvider` already proved.
- **Each provider opens its own `travel_search_backend` span**, nested inside the tool span — giving the trace shape the spec's own diagram asks for (`agent → execute_tools → search_hotels → travel_search_backend → agent`) and an isolated latency signal for the search backend, independent of LLM latency, for free from the span's own duration. Both providers use the *same* span name (`metadata.backend` distinguishes them) — the same "structural trace-shape consistency across providers" reasoning `MockProvider`/`AnthropicProvider` already established for `llm_call`.
- **The HTTP contract `TravelAISearchAPIProvider` assumes is designed, not confirmed** — this repo has no access to the real Travel AI Search project. Response bodies are validated against this project's own `Destination`/`Hotel` models; a real deployment with a different shape raises a clear validation error rather than silently accepting malformed data. Pinned end-to-end by `tests/integration/test_travel_ai_search_provider.py` against a real local HTTP server implementing exactly this assumed contract — not skip-by-default like the Anthropic integration test, since no paid credential is needed, only a loopback socket.
- **On backend failure, no new error-handling code was needed** — a real HTTP error or connection failure from `TravelAISearchAPIProvider` propagates up unhandled, and `tools_node`'s existing per-call `try/except` (M6) already turns it into a graceful error message the agent's next turn can react to, the same recovery path M15's `tool_exception` fault already exercises. Closes the loop M15's own RATIONALE entry left open ("'Travel provider error' has no real second provider to demonstrate a fallback *to*... isn't fully exercisable here yet").
- **A real circular import, found while wiring this up**: `tools/travel_tools.py` needs `providers.travel_search`; `providers/travel_search/local.py` needed `tools.data`; but importing anything from `tools` first runs `tools/__init__.py`, which eagerly imports `travel_tools.py` back again. Fixed by moving the JSON loader to `providers/travel_search/data.py` — also the more honest home for it, since loading the local dataset is now a concern of the *local search provider*, not the tool-wrapping layer above it.
- **Deliberately not built**: automatic fallback from `travel_ai_search_api` back to `local` on failure — the spec asks for "service composition without creating a hard dependency" (the app must still run on `local` alone), not automatic failover mid-request, which would need a policy for partial results and retries the spec never asks for.

### Langfuse Cloud (M19)

"Document and test optional Langfuse Cloud configuration... verify the same application can send traces either to local self-hosted Langfuse or Langfuse Cloud... do not duplicate instrumentation code" — the spec's own three requirements. See [ADR-004](decisions/ADR-004-langfuse-deployment.md) for the original design decision and [docs/RATIONALE_PER_MILESTONE.md](RATIONALE_PER_MILESTONE.md#milestone-19--langfuse-cloud) for what this milestone actually found.

- **No new instrumentation code was needed — verifying that was the milestone's real job.** `get_langfuse_client()` has had exactly one `host=settings.langfuse_host` line, with no branching, since Milestone 1. The three-env-var switch (`LANGFUSE_PUBLIC_KEY`/`_SECRET_KEY`/`_HOST`) was already correct by construction; nothing before this milestone had actually *proven* it.
- **`tests/integration/test_langfuse_connectivity.py` is the verification mechanism itself**, not a separate Cloud-specific test file — reframed (not rewritten) to make explicit it runs unmodified against whichever target `Settings.langfuse_host` currently names. One new assertion (`trace_url.startswith(get_settings().langfuse_host)`) closes the loop between "config says X" and "the trace actually landed at X."
- **`tests/unit/test_langfuse_client.py::test_construction_does_not_raise_with_a_cloud_shaped_host`** pins the offline-verifiable half: construction never depends on the host's value, whether that host is reachable or not. Deliberately does not inspect SDK internals (`client._base_url`) — this file's own stated principle, predating this milestone.
- **Verified live against this environment's own real, non-default Langfuse host** (`http://localhost:3001`, not the `3000` default — a real port conflict from Milestone 1) — a genuine, if not-actually-Cloud, proof that the mechanism generalizes to *any* configured host, not just the one everyone's `.env` happens to use by default.
- **Not exercised against a real Langfuse Cloud account** — none is configured in this environment, the same "no credential here" gap as the Anthropic and real-judge integration tests. The mechanism is proven structurally and would work the moment real Cloud credentials exist; see [docs/EXPERIMENTS.md](EXPERIMENTS.md), Milestone 19, for exactly what was and wasn't run live.

### Final Experiment Suite (M21)

A representative configuration matrix reported across every dimension the spec's own list names — deterministic score, LLM judge score, human feedback where available, tool accuracy, groundedness, latency, cost — closing with an auto-generated engineering recommendation. See [docs/RATIONALE_PER_MILESTONE.md](RATIONALE_PER_MILESTONE.md#milestone-21--final-experiment-suite) and [docs/EXPERIMENTS.md](EXPERIMENTS.md) (Milestone 21) for the real 4-config run.

- **`evaluation/final_suite.py`** — `ConfigSuiteResult`, `run_config_suite()` (pure aggregation, no I/O), `render_final_suite_report()`, `render_final_analysis()`. Every metric it reports is reused, not reinvented: `compute_config_metrics()` (M14/M17) for quality/trajectory/latency/cost, `summarize_trajectories()` (M13) for tool precision/recall, Layer 1's `response_references_tool_result` evaluator (M9) isolated for the groundedness proxy, `summarize_judgments()` (M11) for the LLM judge. "Human feedback" is a fixed explanatory note, not a fabricated number — `POST /feedback` (M12) scores real, live traffic; no human has rated any of the 39 offline cases.
- **`scripts/run_final_experiment_suite.py`** (`make final-experiment-suite`) — a 4-config matrix crossing `PROMPT_LABEL` (production/staging, M8) against `AGENT_MAX_ITERATIONS` (1/5, M14/17), substituting for the spec's own "prompt version x two models" example since a second real model needs `ANTHROPIC_API_KEY`, unavailable in this environment.
- **A real bug found before the suite ran once**: `run_case_with_metrics()` (M14) hardcoded the `"cost-latency-experiment"` Langfuse tag — reusing it unchanged would have mislabeled every M21 trace. Fixed with a backward-compatible `experiment_tag` parameter; verified live that traces now carry `final-experiment-suite`, not the old tag.
- **A real methodological validation**: the two prompt-version pairs matched to two decimal places on every metric (`MockProvider` never reads the system prompt — confirmed by reading `_decide()`, not assumed), while `AGENT_MAX_ITERATIONS` reproduced Milestone 14's own quality/trajectory gap exactly. A harness producing identical numbers for known-identical conditions and a real gap for known-different ones has passed a genuine validity check.
- **Deliberately not built**: a real second-model or "improved tool descriptions" comparison — both structurally inert under Mock for the same reason the prompt-version axis is, so building either would only reconfirm the same fact.
...
`src/travel_ai_concierge/observability/langfuse_client.py` builds the Langfuse client explicitly from `Settings` (`get_langfuse_client()`), rather than relying on the SDK's own env-var auto-discovery — see [ADR-004](decisions/ADR-004-langfuse-deployment.md) and [RATIONALE_PER_MILESTONE.md](RATIONALE_PER_MILESTONE.md#milestone-1--local-langfuse) for why. There is exactly one `host=settings.langfuse_host` line, with no branching on its value — the same three env vars switch between local and Cloud with zero code changes, tested (not just asserted) as of Milestone 19: `tests/integration/test_langfuse_connectivity.py` runs unmodified against whichever target is configured, and additionally asserts the returned trace URL actually starts with the configured host. See [RATIONALE_PER_MILESTONE.md](RATIONALE_PER_MILESTONE.md#milestone-19--langfuse-cloud).
...
Travel search provider selection (`TRAVEL_SEARCH_PROVIDER=local|travel_ai_search_api`, Milestone 18) is the same pattern one layer down — `TRAVEL_AI_SEARCH_BASE_URL`/`TRAVEL_AI_SEARCH_TIMEOUT_SECONDS`/`TRAVEL_AI_SEARCH_API_KEY` configure the optional backend, unused unless selected. See [ADR-006](decisions/ADR-006-travel-search-provider-abstraction.md).
...
The synthetic travel dataset (`data/synthetic/*.json`) is not `Settings`-configurable — its path is resolved relative to the `providers/travel_search/data.py` module's own location (moved from `tools/data.py` in Milestone 18), since it's a fixed part of this repository rather than an environment-specific value. Regenerate it with `make generate-data` after editing `scripts/generate_data.py`.
...
| M18       | Optional Travel AI Search integration | ✅ Complete |
| M19       | Langfuse Cloud                       | ✅ Complete |
| M20       | Production observability architecture | ✅ Complete |
| M21       | Final experiment suite               | ✅ Complete |

All 22 milestones (M0–M21) in `docs/PROJECT_SPEC.md` are complete.
...
| [ADR-006](decisions/ADR-006-travel-search-provider-abstraction.md) | Protocol-based Travel Search provider abstraction (local synthetic default, optional Travel AI Search API) |

## Production Observability Architecture (M20)

How this educational system's architecture would evolve toward a real production deployment — API metrics, logs, distributed tracing, OpenTelemetry, Prometheus/Grafana or cloud APM, alerting, trace sampling, data retention, PII, secrets, scaling, high availability, asynchronous ingestion, multi-region, all grounded in verified facts about *this* codebase (e.g. only two log lines exist in the whole app; `ConversationStore` is the first thing that breaks on horizontal scaling; zero routes have authentication) rather than generic cloud-architecture advice. Documentation only, per the spec's own instruction — no new infrastructure was built. See [docs/PRODUCTION_ARCHITECTURE.md](PRODUCTION_ARCHITECTURE.md) and [docs/RATIONALE_PER_MILESTONE.md](RATIONALE_PER_MILESTONE.md#milestone-20--production-observability-architecture).
